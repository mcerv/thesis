...
void plotDefaultMeas() {


  ifstream inFile;
  inFile.open(Form("data/ampl/ampl-7bit_scope.csv"));
  ...
  string picPath;
  char delim;
  double runNumber[arrlen], noiseDb[arrlen], noiseDbStd[arrlen], noise[arrlen], noiseStd[arrlen], noiseAvg[arrlen],
  ...
  while (1) {
      //reading results
      inFile >> runNumber[nSample] >> delim;
  ...
  inFile.open(Form("data/ampl/ampl-7bit_PSA.csv"));
  nSample = 0;
  // first line
  cout << "Lines:" << endl;
  for (int i=0; i<14; i++) {
