...
void Plotter::initHist_v5()
{
  ...
  _extHistArea_v5 = new TH1D ("ExtArea", "ExtArea", NBINS_EXTAREA,0,NBINS_EXTAREA-1);
...
void Plotter::setMonitorValues (int32_t fwhmThres=0,
  ...
  _widthHistScale=widthHistScale;
}
...
void Plotter::initHist_v6()
{
  ...
  _extHistFwhmSlope_v6 = new TH2D ("Width vs Slope", "Width vs Slope",
                                   2*NBINS_FWHM_SLOPE_X, -NBINS_FWHM_SLOPE_X, NBINS_FWHM_SLOPE_X-1,
                                   2*NBINS_FWHM_SLOPE_Y, -NBINS_FWHM_SLOPE_Y, NBINS_FWHM_SLOPE_Y-1);
  _leg = new TLegend (0.65,0.75,0.9,0.9);
}


void Plotter::fillHist_v6(int32_t USBdatalen, uint8_t USBdata[]) {
  ...
  for (int32_t i = 0; i < USBdatalen-32; i = i + 32) {//read in 256 bits at a time.
    ...
    if (bin>NBINS_EXTAREA-1) continue;
    ...
    exthistalphaarea_v5[bin] =  (uint32_t)(USBdata[i+4+23] << 0 ) |
                                ...
                                (uint32_t)(USBdata[i+4+21] << 16) |
                                (uint32_t)(USBdata[i+4+20] << 24);
    //cout<<" extarea   "<<exthistarea_v5[bin];
    //cout<<" extalar   "<<exthistalphaarea_v5[bin];
    ...
    exthistfwhmslope_v6[bin] =  (uint32_t)(USBdata[i+4+19] << 0 ) |
                                ...
                                (uint32_t)(USBdata[i+4+17] << 16) |
                                (uint32_t)(USBdata[i+4+16] << 24);
    //cout<<" fwhmslope "<<exthistfwhmslope_v6[bin];

    // cout<<" bin "<<bin
    //     <<" fwsl "<<exthistfwhmslope_v6[bin]
    //     <<endl;

    if (bin>NBINS_AREA-1) continue;
    ...
    histalphamaxampl[bin] =   (uint32_t)(USBdata[i+4+11] << 0 ) |
                              ...
                              (uint32_t)(USBdata[i+4+9 ] << 16) |
                              (uint32_t)(USBdata[i+4+8 ] << 24);
    //cout << " bin " << bin << " , ampl      "<<histmaxampl[bin] << endl;
    //cout<<" amplalpha "<<histalphamaxampl[bin];
    //cout<<" width     "<<histfwhm[bin];
  ...
  //saveMaxAmplitudeHistogram(histmaxampl);
  ...
  for (int32_t y = 0; y < NBINS_FWHM_SLOPE_Y; y++) {
    ...
    _extHistFwhmSlope_v6->Fill(x,y, exthistfwhmslope_v6[y*NBINS_FWHM_SLOPE_X + x]);
    // cout<<" x "<<x<<" y "<<y<<" value "<<exthistfwhmslope_v6[y*NBINS_FWHM_SLOPE_X + x]<< endl;
    }
  }
...
void Plotter::saveMaxAmplitudeHistogram(uint32_t * histmaxampl) {

    static int saveFileCounter = 0;


    // if(saveFileCounter % 10 == 0)
    //     std::cout << saveFileCounter << std::endl;
    //
    // if(saveFileCounter > 10)
    // {
    //     saveFileCounter++;
    //     return;
    // }
    ...
    myfile.open(name.c_str(), std::ios::out);

    //myfile << 256 << "\n" ;

    for(int i = 0; i < 128; i++)
    {
        myfile << i << "\t   ,   " << ((double)histmaxampl[i]) << "\n";
    }
...
void Plotter::showHist_v6() {
  ...
  // double Area_MeV = ADC_pVs*ENERGY_SCALING_FACTOR_PVS_TO_MEV;
  double ADC_mV = _inputRange/128; //in mV
  double width_ns = NBINS*SAMPLING_RATE*pow(2,(double)_widthHistScale);
  double ADC_pVs = ADC_mV*SAMPLING_RATE*pow(2,(double)_areaHistScale);
  double extADC_pVs = ADC_mV*SAMPLING_RATE*pow(2,(double)_areaHistScale);
  double Area_MeV = ADC_pVs*ENERGY_SCALING_FACTOR_PVS_TO_MEV;
  double extArea_MeV = extADC_pVs*ENERGY_SCALING_FACTOR_PVS_TO_MEV;
  //cout<<" _inputRange = "<<_inputRange;
  //cout<<" ADC_mV      = "<<ADC_mV;
  ...
  gPad->SetLogy(true);
  _histFwhm_v4->Draw();
  _histFwhm_v4->GetXaxis()->SetLimits(0,width_ns);
  _histFwhm_v4->GetXaxis()->SetTitle("FWHM [ns]");
  _histFwhm_v4->GetYaxis()->SetTitle("Entries");
  _histFwhm_rej_v4->SetFillColor(kBlue-8);
  _histFwhm_rej_v4->SetLineColor(kBlue-8);
  _histFwhm_rej_v4->SetFillStyle(3004);
  _histFwhm_rej_v4->Draw("same");
  _histFwhm_rej_v4->GetXaxis()->SetLimits(0,width_ns);
  _histFwhm_acc_v4->SetFillColor(kRed+1);
  _histFwhm_acc_v4->SetLineColor(kRed+1);
  _histFwhm_acc_v4->Draw("same");
  _histFwhm_acc_v4->SetFillStyle(3005);
  _histFwhm_acc_v4->GetXaxis()->SetLimits(0,width_ns);
  ...
  _can->cd(3);
  gPad->SetLogy(false);
  _extHistFwhmSlope_v6->Draw("colz");
  ...
  //_extHistArea_v5->GetXaxis()->SetLimits(0,NBINS_EXTAREA*extArea_MeV);
  _extHistArea_v5->GetXaxis()->SetLimits(0,NBINS_EXTAREA*extADC_pVs);
  _extHistArea_v5->GetXaxis()->SetTitle("Energy [pVs]");
  _extHistArea_v5->GetYaxis()->SetTitle("Entries");
  //_extHistArea_rej_v5->Rebin(2);
  ...
  _extHistArea_acc_v5->SetLineWidth(1);
  _extHistArea_acc_v5->SetFillStyle(3005);
  _extHistArea_acc_v5->Draw();
//    _extHistArea_v5->Draw("same");

  _extHistArea_acc_v5->GetXaxis()->SetLimits(0,NBINS_EXTAREA*extADC_pVs);
  _extHistArea_acc_v5->GetXaxis()->SetTitle("Energy [pVs]");
  _extHistArea_acc_v5->GetYaxis()->SetTitle("Entries");
  //_extHistArea_acc_v5->GetXaxis()->SetLimits(0,NBINS_EXTAREA*extArea_MeV);
  ...
  //-------------- Gaussian fit of the area --------------
  fit1->SetRange(0,1200);
  //_extHistArea_acc_v5->Fit(fit1, "RQ");
  cout<<" AREA results: "<<endl
      <<"  MEAN(Y)  "<<fit1->GetParameter(0)
      <<"  MEAN(X)  "<<fit1->GetParameter(1)
      <<"  SIGMA(X) "<<fit1->GetParameter(2)
...
//-------------------------------------------------------------------
//                        HISTOGRAMMING Ampl Cx
//-------------------------------------------------------------------

void Plotter::initHistAmpl()
{
  //fill all histograms
  if (_histMaxampl_v4) delete _histMaxampl_v4;
  _histMaxampl_v4 = 0;
...
void Plotter::fillHistAmpl(int32_t USBdatalen, uint8_t USBdata[]) {
  ...
  for (int32_t i = 0; i < USBdatalen-32; i = i + 32) {//read in 256 bits at a time.
    ...
    if (bin>NBINS_MAXAMPL-1) continue;

    histmaxampl[bin] =  (uint32_t)(USBdata[i+7] << 0 ) |
                        (uint32_t)(USBdata[i+6] << 8 ) |
                        (uint32_t)(USBdata[i+5] << 16) |
                        (uint32_t)(USBdata[i+4] << 24);
    if (histmaxampl[bin]!=0)
      cout << " bin " << bin << " , ampl      "<<histmaxampl[bin] << endl;
  ...
  for (int32_t i = 1; i < NBINS_MAXAMPL; i++) {
    _histMaxampl_v4->Fill(i, histmaxampl[i]);
  }
...
void Plotter::showHistAmpl() {
  ...
  //-------------- Gaussian fit of the amplitude --------------
  // fit1->SetRange(0,20000);
  // _histMaxampl_v4->Fit(fit1, "RQ");
  // cout<<" AMPLITUDE results: "<<endl
  //     <<"  MEAN(Y)  "<<fit1->GetParameter(0)
  //     <<"  MEAN(X)  "<<fit1->GetParameter(1)
  //     <<"  SIGMA(X) "<<fit1->GetParameter(2)
...
void Plotter::fillWaveformC2(int32_t USBdatalen, uint8_t USBdata[262000])
{
  // //print the buffer
  // for (int32_t i = 0; i < USBdatalen-32; i = i + 32)
  // {
  ...
  for (int32_t i = 0; i < USBdatalen-32; i = i + 4) //read in 32 bits at a time.
  {
    ...
    if (trailer) {
      //read fwhm width, max amplitude and classification
      derivSlope =   (int8_t)(USBdata[i+1]<<2)  <<  0;
      fwhmWidth =    (int64_t)USBdata[i+2]  <<  8 |
                     (int64_t)USBdata[i+3]  <<  0;
      maxAmplitude = (int64_t)USBdata[i+4]  <<  8 |
                     (int64_t)USBdata[i+5]  <<  0;
      cutFlags =       (int64_t)USBdata[i+6]    << 8 |
                       (int64_t)USBdata[i+7]    << 0;
      classification = (int64_t)USBdata[i+8]   <<  8 |
                       (int64_t)USBdata[i+9]   <<  0;
      pulseArea = (int64_t)USBdata[i+10]  <<  8 |
                  (int64_t)USBdata[i+11]  <<  0;

      derivSlope = derivSlope/2;
      i = i + 12;//to end the trailer and proceed to a new header
      trailer = false;
      cntTrailer = 0;
    ...
    if (header) {
      //read timestamp and trigger counter
      // wfLen     = (int64_t)USBdata[i]    << 0;
      // geel_trig = (int64_t)USBdata[i+1]  << 8 |
                  // (int64_t)USBdata[i+2]  << 0;
      timestamp = (int64_t)USBdata[i+3]  << 32 |
                  (int64_t)USBdata[i+4]  << 24 |
                  (int64_t)USBdata[i+5]  << 16 |
                  (int64_t)USBdata[i+6]  <<  8 |
                  (int64_t)USBdata[i+7]  <<  0;
    ...
    for (int32_t j = 0; j < 4; j+=2) {
      if(!header) { // only check for header if not yet in header
        if (USBdata[i+j] == 0xAA && USBdata[i+j+1] == 0xFF )
          cntHeader++;
        else
          cntHeader = 0; //reset the counter if it's not header
        if (cntHeader == 8) {
      ...
      if(!trailer) {
        if (USBdata[i+j] == 0xEE && USBdata[i+j+1] == 0xAA) {
          cntTrailer++;
          maybeTrailer = true;
        }
        else
...
void Plotter::fillWaveform(int32_t USBdatalen, uint8_t USBdata[262000])
{
  ...
  bool data = false;
  //Read through buffer and find information on pulses
  for (int32_t i = 0; i < USBdatalen-128; i = i + 4) //read in 32 bits at a time.
  {
    ...
    if (trailer) {
      ...
      //   cout<<hex;
      //   cout<<" "<<(int64_t)USBdata[i+j];
      //   cout<<dec;
      // }
      // cout<<endl;

      maxAmplitude = (int64_t)USBdata[i+2]  <<  8 |
                     (int64_t)USBdata[i+3]  <<  0;
      trailer = false;
      cntTrailer = 0;
    ...
    if (header) {
      //read timestamp and trigger counter
      waveformLength = (int64_t)USBdata[i+2]  <<  8 |
                       (int64_t)USBdata[i+3]  <<  0;
      // trigCount = (int64_t)USBdata[i+8]  << 56 |
      //             (int64_t)USBdata[i+9]  << 48 |
      //             (int64_t)USBdata[i+10] << 40 |
    ...
    //----------search for header/trailer--------------
    if (USBdata[i] == 0xBB && USBdata[i+1] == 0xFF ) { //test if header
      header = true;
      pulseOverflow = false;
      ofCnt = 0;
      i-=4;
    ...
    if (USBdata[i] == 0xFF && USBdata[i+1] == 0xAA) { //test if trailer
      trailer = true;
      pulseOverflow = false;
      data = false;
      i-=4;
