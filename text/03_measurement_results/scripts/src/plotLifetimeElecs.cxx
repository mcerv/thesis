...
int32_t main (void) {
  ...
  int32_t nFilesIndividual[nTemp] = { 142, 1};
  int32_t numSkip[nTemp] = { 20, 1};
  ...
  int32_t toPlot[3] = {0, 100, 140}; //0, 15 and 30 min
    ...
    for (int32_t temp=0; temp<nTemp; temp++) {
      for (int32_t num=0; num<3; num++) {
        ...
        sssEHsam << "#splitline{#splitline{" << timet << "}{"
                 << "U_{bias}=" << voltage[polarity[sample]-1] << "V}}{"
