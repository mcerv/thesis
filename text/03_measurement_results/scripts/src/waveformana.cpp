...
WaveformAna::WaveformAna () :
  ...
  _cutMinAmplitude(0)
{
  cout<<" Waveform analysis running - minimum."<<endl;
}
...
void WaveformAna::loadLandau(TH1F landau) {
  _histAmpl = new TH1F();
  *_histAmpl = landau;
}

void WaveformAna::fitLandau(float& mpv, float& mpve) {
  mpve = _ptr->Parameter(0);
  mpv = _ptr->Parameter(1);
}
...
int32_t WaveformAna::updateHistos(string saveHistos)
{
  ...
  if (      ( _showHist && !(_numEvents%300) )
       ...
       || ( !saveHistos.compare("SHOW") )
        || ( !saveHistos.compare("SHOW CUT") ) )
  {
    _cHist = new TCanvas("c2","c2",800,600);
    ...
    // Initial Parameters
    mpvFit->SetParameter(0, 0.001);
    mpvFit->SetParameter(1, 0.007);
    mpvFit->SetParameter(2, 20); //?
    mpvFit->SetParameter(3, 0.001);
  ...
  double min = 0;
  double max = 0.1;
  if (!saveHistos.compare("SHOW CUT")) {
    min = 0.014;
  }

  ptr  = _histAmpl->Fit("mpvFit", "SQR","",min,max);//,"",min, max);
  _ptr = ptr;
  cout << " Landau width " << setw(5) << ptr->Parameter(0) << endl;
  cout << " MPV " << setw(5) << ptr->Parameter(1) << endl;
  cout << " Norm " << setw(5) << ptr->Parameter(2) << endl;
  cout << " Gaus width " << setw(5) << ptr->Parameter(3) << endl;
