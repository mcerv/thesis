...
int32_t main (void) {
  ...
  double voltage[nVoltages] = {700.0, 500.0, 400.0, 300.0, 200.0, -200.0, -300.0, -400.0, -500.0, -700.0};
  ...
  for (int32_t sample=S52_363e14; sample<nSamples; sample++) {
    for (int32_t temp=0; temp<nTemp; temp++) {
      ...
      for (int32_t i=0; i<nPol; i++) {
        ...
        leg2[sample][temp][i] = new TLegend(0.6,0.7,0.95,0.95);
  ...
  for (int32_t i=0; i<nPol; i++) {
    ...
    for (int32_t sample = S52_363e14; sample < nSamples; sample++) {
      ...
      for (int32_t temp = 0; temp < nTemp; temp++) {
        ...
        gr[sample][temp][volt]->SetLineColor(color2[temp]);
        gr[sampleNonIrrad][temp][volt]->SetLineColor(color2[temp+2]);
  ...
  for (int32_t sample=S52_363e14; sample<nSamples; sample++) {
    ...
    for (int32_t temp = 0; temp < nTemp; temp++) {
      ...
      for (int32_t i=0; i<nPol; i++) {
        ...
        mg2[sample][temp][i]->GetXaxis()->SetRangeUser(-2,17);
  ...
  for (int32_t sample=0; sample<nSamples; sample++) {
    for (int32_t temp=0; temp<nPlottedTemps; temp++) {
      ...
      for (int32_t i=0; i<nPol; i++) {
        ...
        leg1[sample][temp][i] = new TLegend(0.7,0.5,0.95,0.95);
    ...
    for (int32_t temp = 0; temp < nPlottedTemps; temp++) {
      ...
      for (int32_t i=0; i<nPol; i++) {
        for (int32_t volt = i*nVoltPol; volt < (i+1)*nVoltPol; volt++) { //neg and pos
          ...
          gr[sample][temp1][volt]->SetLineColor(color2[volt]);
  ...
  for (int32_t sample=0; sample<nSamples; sample++) {
    for (int32_t temp=0; temp<nPlottedTemps; temp++) {
      ...
      for (int32_t i=0; i<nPol; i++) {
        ...
        mg1[sample][temp][i]->GetXaxis()->SetRangeUser(-2,17);
  ...
  c1[nSamples-1][nTemp-1][nPol-1]->WaitPrimitive();
...
getchar();
  ...
  for (int32_t sample=0; sample<nSamples; sample++) {
    for (int32_t i=0; i<nPol; i++) {
      ...
      leg[sample][i] = new TLegend(0.7,0.5,0.95,0.95);
  ...
  for (int32_t i=0; i<nPol; i++) {
    ...
    for (int32_t sample = 0; sample < nSamples; sample++) {
      for (int32_t temp = 0; temp < nTemp; temp++) {
        if (!(temp==0 || temp==4 || temp==8  || temp==9  || temp==10  || temp==13  || temp==17)) {
          continue;
        }
        dr->prettify(gr[sample][temp][volt]);
        gr[sample][temp][volt]->SetLineColor(color2[temp]);
  ...
  for (int32_t sample=0; sample<nSamples; sample++) {
    ...
    for (int32_t i=0; i<nPol; i++) {
      ...
      mg[sample][i]->GetXaxis()->SetRangeUser(-2,17);
