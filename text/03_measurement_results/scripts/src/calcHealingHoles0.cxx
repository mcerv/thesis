...
// ------------------------------ MAIN PROGRAM ----------------------
int32_t main (void) {
  cout<<"Calculating HEALING 4 ... HOLES"<<endl;
  ...
  int32_t nFilesIndividual[nTemp] = { 1, 1, 1, 1,1,1,1,1,1};
  int32_t numSkip[nTemp] = { 1, 1, 1, 1,1,1,1,1,1};

  string tempS[nTemp] = { "1", "2", "3","4", "5", "6", "7", "8", "9"};

  double delay[nTemp] = {-1, 1, 180, 600, 1800, 3600, 4*3600, 12*3600, 20*3600}; //seconds. -1 because it's a clean pulse
  ...
  ofstream fout ("data/healing0.txt");
  ...
  for (int32_t sample = 0; sample < nSamples; sample++) {
    for (int32_t temp = 0; temp < nTemp; temp++) {
      for (int32_t num = 0; num < nFilesIndividual[temp]; num+=numSkip[temp]) {
        ...
        fout << num << "\t" << delay[temp] << "\t" << correlation (pulse[sample][temp][num], pavg ) << endl;
